...
export const createGame = (client, ownerId, playerIds, wordsPerMove, totalMovesAllowed) => {
	...
	`;
	return client.query(queryString, [ownerId, playerIds, wordsPerMove, totalMovesAllowed, new Date().toISOString()]);
};

export const updateCurrentMoveIndex = (client, gameId, newIndex) => {
	return client.query('UPDATE games SET current_move_index = $1 WHERE id = $2',
		[newIndex, gameId]
	);
}
...
export const createMove = (client, gameId, playerId, indexInStory, words) => {
	const queryString = `
	INSERT INTO moves(game_id, player_id, index_in_story, words, created_at)
	VALUES($1, $2, $3, $4, $5) RETURNING *;
	`;
	return client.query(queryString, [gameId, playerId, indexInStory, words, new Date().toISOString()]);
};
